...
def octant_analysis(mod=5000):
    ...
    os.chdir(r'G:\CS384\CS384_2022\tut07\input')
    lst_files=glob.glob('*.xlsx')
    sat=0
    for file in lst_files :

        os.chdir(r'G:\CS384\CS384_2022\tut07\input') #again we changing the dir to input from output
        ...
        for i in range(8):
            ...
            if (8-i == 1):
                ...
                df1.loc[0, "Rank1 Octant Name"] = octant_name_id_mapping[str(int(df1.loc[0, "Rank1 Octant ID"]))]
        ...
        os.chdir(r'G:\CS384\CS384_2022\tut07\output')
        print(inp)        
        df1.to_excel(inp, index=False)
        ...
        for i in range (row_loc,row_loc+row_num): # for mod
            for j in range (col_loc,col_num+col_loc):
                ...
                # if i==row_loc:
                #     ws.cell(row=i+1, column=j+1).border=Double_border
                #     ws.cell(row=i+1, column=j+1).fill=fill_cell
        ...
        for i in range (row_loc,row_loc+row_num): # for mod
            # if(ws.cell(row=i, column=col_loc+1).value == None):
            #     break
            for j in range (col_loc,col_num+col_loc):
                ws.cell(row=i, column=j).border=thin_border            
