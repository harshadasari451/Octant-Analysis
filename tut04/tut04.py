...
try:
   ...
   def octant_longest_subsequence_count_with_range():
    try:      
      df = pd.read_excel("input_octant_longest_subsequence_with_range.xlsx")  # reading the input file
      ...
      for i in range(mod) :
         ...
         if a[int(df["Octant"][i])+4] != a[int(df["Octant"][i+1])+4]:
            if(a[int(df["Octant"][i])+4]>b[int(df["Octant"][i])+4]):
              ...
              if (a[int(df["Octant"][i])+4]<b[int(df["Octant"][i])+4]):
                  a[int(df["Octant"][i])+4] =0
      df.loc[0,"Longest Subsequence Length"] = b[5]           # storing the maximum length of subsequence +1 in b[5]
      df.loc[1,"Longest Subsequence Length"] = b[3]           # storing the maximum length of subsequence -1 in b[3]
      df.loc[2,"Longest Subsequence Length"] = b[6]           # storing the maximum length of subsequence +2 in b[6]
      df.loc[3,"Longest Subsequence Length"] = b[2]           # storing the maximum length of subsequence -2 in b[2]
      df.loc[4,"Longest Subsequence Length"] = b[7]           # storing  the maximum length of subsequence +3 in b[7]
      ...
      w=0
      for i in range(8) :
          df.loc[w,"OCTANT"]=df["octant"][i]#copying the data in octant to OCTANT column
          df.loc[w,"longest subsequence length"]=df["Longest Subsequence Length"][i] #copying the data in Longest subsequence length  to longest subsequence length column
